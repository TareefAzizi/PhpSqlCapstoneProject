            ...
            switch( $condition ) 
            {
                ...
                case 'edit_user_form_csrf_token':
                    ...
                    case 'delete_post_form_csrf_token':
                        ...
                        case 'edit_products_form_csrf_token':
                            // $data[$key] is $_POST['csrf_token'];
                            if ( !CSRF::verifyToken( $data[$key], 'edit_products_form' ) ) {
                                $error .= "Invalid CSRF Token<br />";
                            }
                            break;

                        case 'add_products_form_csrf_token':
                            // $data[$key] is $_POST['csrf_token'];
                            if ( !CSRF::verifyToken( $data[$key], 'add_products_form' ) ) {
                                $error .= "Invalid CSRF Token<br />";
                            }
                            break;
                            
                        case 'delete_products_form_csrf_token':
                            // $data[$key] is $_POST['csrf_token'];
                            if ( !CSRF::verifyToken( $data[$key], 'delete_products_form' ) ) {
                                $error .= "Invalid CSRF Token<br />";
                            }
                            break;
